...
final class ReminderListEditingViewController: CustomNavigationChildViewController {
    ...
    public init(list: EKCalendar) {
        
        self.list = list
        self.mode = list.isNew ? .create : .edit
    ...
    private func setNavigationItems() {
        ...
        titleLabel.text = mode == .create ? Strings.newList : Strings.editList
    ...
    private func checkAndSetDoneButtonEnabled() {
        ...
        if list.title.isEmpty || list.cgColor == nil {
            doneButton.isEnabled = false
        } else {
            if list.isNew {
                doneButton.isEnabled = true
            } else if list.hasChanges {
                doneButton.isEnabled = true
            } else {
                doneButton.isEnabled = false
            }
    ...
    @objc private func cancelButtonPressed() {
        ...
        if mode == .edit {
            ...
            calendarItemsManager.deleteReminderList(list)
        }
    ...
    @objc private func saveButtonPressed() {
        ...
        calendarItemsManager.saveReminderList(list)
    ...
    @objc private func colorTextFieldTextDidChange(_ sender: UITextField) {
        ...
        guard color.cgColor != list.cgColor else { return }
        
        list.cgColor = color.cgColor
        checkAndSetDoneButtonEnabled()
    }
...
extension ReminderListEditingViewController: UICollectionViewDataSource {
    
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        
        if mode == .create || list.type == .birthday {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .TitleCell:
            ...
            configuration.textField.text = list.title
            if list.isNew {
        ...
        case .ColorCell:
            ...
            var color: UIColor {
                if let calendarColor = list.cgColor {
        ...
        case .DeleteCell:
            ...
            configuration.label.text = list.type == .subscription ? Strings.unsubscribe : Strings.deleteList
...
extension ReminderListEditingViewController: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        switch cellName {
        case .DeleteCell:
            let alertTitle = list.type == .subscription ? Strings.sureToUnsubscribeFrom + " \"\(list.title)\"" + Strings.questionMark : Strings.sureToDelete + " \"\(list.title)\"" + Strings.questionMark
            let alert = UIAlertController(title: alertTitle, message: nil, preferredStyle: .actionSheet)
            let deleteActionTitle = list.type == .subscription ? Strings.unsubscribe : Strings.deleteList
...
extension ReminderListEditingViewController: UIAdaptivePresentationControllerDelegate {
    ...
    func presentationControllerShouldDismiss(_ presentationController: UIPresentationController) -> Bool {
        ...
        switch mode {
        case .create:
            return list.title.isEmpty && list.cgColor == nil
        case .edit:
...
extension ReminderListEditingViewController: UITextFieldDelegate {
