...
final class EventCalendarChoosingViewController: CalendarItemEditingChildViewController {
    ...
    private var calendars: [EKCalendar] {
        return (calendarItemsManager.currentVisibleEventCalendars + calendarItemsManager.currentInvisibleEventCalendars).filter { $0.allowsContentModifications }
    }
    private var sources: [EKSource] {
        let set = Set(calendars.map({ $0.source! }))
        return Array(set)
    }
...
extension EventCalendarChoosingViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        return calendars.filter({ $0.source == source }).count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        
        let calendar = calendars.filter({ $0.source == sources[indexPath.section] })[indexPath.item]
        ...
        configuration.image = Constants.SFSymbols.circlebadgeFill.withConfiguration(UIImage.SymbolConfiguration(scale: .default)).withTintColor(UIColor(cgColor: calendar.cgColor), renderingMode: .alwaysOriginal)
        configuration.text = calendar.title
        
        cell.contentConfiguration = configuration
        
        cell.accessories = [.checkmark(options: .init(isHidden: calendar != event.calendar))]
...
extension EventCalendarChoosingViewController: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        guard event.calendar != calendar else { return }
        ...
        eventEditingViewController.reloadItem(at: IndexPath(item: 0, section: 0))
